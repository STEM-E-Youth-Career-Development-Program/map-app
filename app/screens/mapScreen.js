...
const MAP_API_KEY = "AIzaSyABw1Zlew8uwDkqokOuy9d7JWo9k4Ykj80"; // Temporary hardcoded value
import {
  useNavigation,
  useFocusEffect,
  useRoute,
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // const eventbriteEnabled = await AsyncStorage.getItem(
        //   "eventbriteEnabled"
        // );
        // const includeEventbrite = eventbriteEnabled !== "false"; // Default to true

        const events = await fetchEvents(false); // Temporarily disable Eventbrite
        setAllEvents(events);
        //console.log("check map screen events", events)
        onsiteEvents = events.filter(
          (event) =>
            event.eventType === "Onsite" && event.eventStatus === "Active" // Removed eventbrite filter temporarily
        );
