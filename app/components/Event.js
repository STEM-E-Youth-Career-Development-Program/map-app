...
function Event({
  ...
  const renderImage = () => {
    if (!image) {
      return (
        <Image style={styles.image} source={require("../assets/STEME.png")} />
      );
  ...
  return (
    <>
      <TouchableOpacity
        activeOpacity={0.5}
        onPress={() =>
          navigation.navigate("Event Details", { allDetails, distance })
        }
        style={styles.container}
      >
        <View style={styles.shadow}>{renderImage()}</View>
        <View style={styles.info}>
          ...
          >
            <Text
              style={[
                styles.heading,
                {
                  maxWidth: allDetails.source === "eventbrite" ? "60%" : "70%",
                },
              ]}
              numberOfLines={2}
              ellipsizeMode="tail"
            ...
            {allDetails.source === "eventbrite" && (
              <View style={styles.eventbriteTag}>
                <Text style={styles.eventbriteText}>Eventbrite</Text>
              </View>
            )}
          ...
          >
            <Text
              style={[styles.subjects, { maxWidth: "70%" }]}
              numberOfLines={1}
              ellipsizeMode="tail"
            >
              Subject : {subject}
        ...
        {allDetails.eventStatus === "Active" &&
          allDetails.eventType === "Onsite" && (
            <TouchableOpacity
              style={{ marginLeft: "auto" }}
              onPress={() => {
                navigation.navigate("Home", { eventId: allDetails.id });
              }}
            >
              <Image
                style={styles.mapPin}
